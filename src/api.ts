...
export interface KnowledgeUploadResponse { // `/knowledge/upload` 上传文件后的响应结构
...
export async function uploadKnowledgeFile(file: File, signal?: AbortSignal) { // 上传一个知识库文件
  ...
  const response = await fetch(`${API_BASE_URL}/knowledge/upload`, { // 请求后端上传接口
  ...
  return response.json() as Promise<KnowledgeUploadResponse> // 成功时返回上传结果
}
