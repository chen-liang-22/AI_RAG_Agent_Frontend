...
async function handleKnowledgeFileChange(event: Event) { // 用户选择文件后上传
  ...
  try {
    const response = await uploadKnowledgeFile(file) // 调用后端上传并入库接口
    if (response.status === 'duplicate') { // 后端判断 MD5 已存在
      ElMessage.info(response.message || '相同内容的文件已经存在') // 展示重复提示
    } else {
      ElMessage.success(response.message || '文件已上传并写入知识库') // 展示成功提示
    }
    await refreshKnowledgeFiles() // 上传后刷新文件列表
    await refreshHealth() // 入库后刷新 Qdrant 状态
  } catch (error) {
    ElMessage.error(error instanceof Error ? error.message : '文件上传失败') // 展示错误
  } finally {
