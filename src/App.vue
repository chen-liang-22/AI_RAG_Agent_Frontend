...
import { computed, nextTick, onMounted, ref } from 'vue' // ref 创建响应式数据；computed 创建派生数据；nextTick 等 DOM 更新；onMounted 页面挂载后执行
import {
  Bot, // 机器人图标
  Clock3, // 聊天记录图标
  DatabaseZap, // 知识库图标
...
const conversationPageSize = 10 // 聊天记录每页 10 条
...
const pagedKnowledgeFiles = computed(() => { // 当前知识库页展示的 9 个文件
  ...
  return knowledgeFiles.value.slice(start, start + knowledgePageSize) // 返回当前页文件
})
...
async function refreshKnowledgeFiles() { // 刷新知识库文件列表
  ...
  try {
    ...
    const maxPage = Math.max(1, Math.ceil(knowledgeFiles.value.length / knowledgePageSize)) // 计算最大页码
...
async function refreshConversations() { // 刷新聊天记录列表
  ...
  try {
    const response = await listConversations(conversationPage.value, conversationPageSize) // 请求聊天记录分页接口
            ...
            <div class="conversation-messages">
              ...
              >
                <div class="history-message-meta">
                  <span>{{ message.role === 'assistant' ? '助手' : message.role === 'user' ? '用户' : message.role }}</span>
