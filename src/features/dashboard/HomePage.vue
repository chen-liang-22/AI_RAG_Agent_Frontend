...
import { displayOptionLabel, safeList, sortTrainingBatchesByImportTime, valueList } from '../../utils/trainingDisplay'
...
const trainingKnowledgePreview = ref<TrainingKnowledgePreviewResponse | null>(null) // 训练资料原文预览
const trainingKnowledgePreviewVisible = ref(false) // 训练资料原文预览弹窗开关
...
const trainingKnowledgeUploadHelpDescription = computed(() => (
  '训练资料上传只负责文件入库、去重、切片预览和发布，不在上传阶段配置画像、行业、难度和评分规则。'
))
...
async function previewTrainingKnowledgeFileBatch(batch: TrainingKnowledgeBatchResponse) { // 预览后端保存的训练资料原文件解析文本
