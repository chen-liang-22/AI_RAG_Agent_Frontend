...
import {
  ...
  reloadKnowledge, // 调用后端知识库重载接口（扫描 data）
...
import { openHttpFilePreview } from '../../../shared/utils/filePreview' // 打开 MinIO HTTP 文件预览地址
...
const reloading = ref(false) // 是否正在重新加载知识库（扫描 data 目录）
...
async function handlePreviewKnowledgeFile(file: KnowledgeFileResponse) { // 预览已入库知识库文件
  ...
  try {
    const preview = await previewKnowledgeDocument(file.document_id) // 后端返回 MinIO HTTP 预览地址
    openHttpFilePreview(preview.file_url, '文件预览地址为空，请确认 MinIO 文件是否存在') // 直接打开上传原文件，不再展示解析文本弹窗
  } catch (error) {
...
async function handleReloadKnowledge() { // 扫描 data 目录并重新加载到 Qdrant
  reloading.value = true
  try {
    await reloadKnowledge()
    ElMessage.success('知识库已重新加载')
